...
class Text:
    def __init__(self, id, content, location):
        ...
        if 'vertices' in location:
            self.width = location['vertices'][1]['x'] - location['vertices'][0]['x']
            self.height = location['vertices'][2]['y'] - location['vertices'][1]['y']
            if len(self.content) > 0:
                self.word_width = self.width / len(self.content)
        else:
            print("")
            # Handle the missing 'vertices' case appropriately here
